...
function SalesDashboard() {
  ...
  const fetchData = useCallback(async (overrideFilters) => {
    ...
    try {
      ...
      const [summaryRes, monthlyRes, itemsRes, tipeItemRes] = await Promise.all([
        axios.get(`${API_BASE}/sales/aggregate/summary`, { params }),
        axios.get(`${API_BASE}/sales/aggregate/monthly`, { params }),
        axios.get(`${API_BASE}/sales/aggregate/top_items_by_qty`, { params: { ...params, limit } }),
        axios.get(`${API_BASE}/sales/aggregate/by_tipe_item`, { params })
      ]);
  ...
  return (
    <div>
      ...
      <div className={`${cardBg} rounded-lg shadow ${borderClass} p-6 mb-6`}>
        <div className="text-sm text-gray-500 dark:text-gray-400">Total Sales Amount</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        {!loading && chartData.length > 0 && (
          <div className={`${cardBg} rounded-lg shadow ${borderClass} p-4`}>
            <h2 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Monthly Sales by Outlet</h2>
        ...
        {!loading && byTipeItem.length > 0 && (
          <div className={`${cardBg} rounded-lg shadow ${borderClass} p-4`}>
            <h2 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Sales by Tipe Item (Amount)</h2>
        ...
        <div className={`${cardBg} rounded-lg shadow ${borderClass} mb-6 overflow-hidden`}>
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-white">Top {filters.top_n} Items by Total Amount</h2>
              <select
                name="top_n_sales"
                value={filters.top_n}
                onChange={(e) => {
